...
class WriteStatement(Statement):
    """Provide common write ops attributes.
    """
    ...
    def patch(self, doc):
        ...
        if not isinstance(doc, (ExprParser, dict, DbDoc, str)):
            raise ProgrammingError(
                "Invalid data for update ops on document collection "
                "table")
...
class ReadStatement(FilterableStatement):
    ...
    def lock_shared(self, lock_contention=LockContention.DEFAULT):
        """Execute a read ops with `SHARED LOCK`. Only one lock can be
           active at a time.
    ...
    def lock_exclusive(self, lock_contention=LockContention.DEFAULT):
        """Execute a read ops with `EXCLUSIVE LOCK`. Only one lock can be
           active at a time.
