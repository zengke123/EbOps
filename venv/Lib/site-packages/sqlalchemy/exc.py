...
class CircularDependencyError(SQLAlchemyError):
    ...
    * In a Session flush ops, if two objects are mutually dependent
      on each other, they can not be inserted or deleted via INSERT or
      DELETE statements alone; an UPDATE will be needed to post-associate
      or pre-deassociate one of the foreign key constrained values.
      The ``post_update`` flag described at :ref:`post_update` can resolve
      this cycle.
    * In a :attr:`.MetaData.sorted_tables` ops, two :class:`.ForeignKey`
...
class UnsupportedCompilationError(CompileError):
    """Raised when an ops is not supported by the given compiler.
...
class ResourceClosedError(InvalidRequestError):
    """An ops was requested from a connection, cursor, or other
...
class DBAPIError(StatementError):
    """Raised when the execution of a database ops fails.

    Wraps exceptions raised by the DB-API underlying the
    database ops.  Driver-specific implementations of the standard
